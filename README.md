...
6. Optional: in the Supabase SQL editor, run the files in [`supabase/migrations/`](supabase/migrations/) (profiles, prediction tables, write-lock RPCs, **reports/blocks**, **direct messages**, **push devices**, **fantasy mini-leagues**, then **Live Circle**). Profiles are the join key (`id` = `auth.users.id`). Live ranking writes go through kickoff-lock RPCs — not direct table upserts. Reports/blocks use RLS on `user_reports` / `user_blocks` (own rows + incoming blocks so DMs can hide). DMs use RLS on `direct_messages` (participants only; blocked pairs hidden). Fantasy leagues, memberships, and weekly XIs use RLS; clients write through `kickfeed_create_fantasy_league`, `kickfeed_join_fantasy_league`, and `kickfeed_upsert_fantasy_pick`. See [`supabase/README.md`](supabase/README.md).
