...
describe('football BFF URL config', () => {
  it('points EAS release profiles at the workers.dev placeholder and development at loopback', () => {
    const eas = JSON.parse(readFileSync(new URL('../../eas.json', import.meta.url), 'utf8')) as {
      build: {
        development: { environment: string; env: { EXPO_PUBLIC_FOOTBALL_BFF_URL: string } };
        preview: { environment: string; env: { EXPO_PUBLIC_FOOTBALL_BFF_URL: string } };
        production: { environment: string; env: { EXPO_PUBLIC_FOOTBALL_BFF_URL: string } };
      };
    };
    ...
    expect(eas.build.preview.env.EXPO_PUBLIC_FOOTBALL_BFF_URL).toBe(PROD_FOOTBALL_BFF_URL_PLACEHOLDER);
    expect(eas.build.production.environment).toBe('production');
    expect(eas.build.production.env.EXPO_PUBLIC_FOOTBALL_BFF_URL).toBe(PROD_FOOTBALL_BFF_URL_PLACEHOLDER);
  });
