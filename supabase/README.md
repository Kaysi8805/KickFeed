...
4. SQL Editor (or `supabase db push`) → apply:
   ...
   - [`migrations/20260919120000_direct_messages.sql`](migrations/20260919120000_direct_messages.sql) — `direct_messages` + incoming-block SELECT + `dm` report target; slow-mode trigger
...
Direct messages: participants select/insert `direct_messages` when they are `sender_id` or `recipient_id` and neither person blocked the other. Server trigger `slow_mode` matches the client 20s / thread + 8 / 2 min burst. Text 1–1000 chars. No groups, no media.

### Apply notes (Karol)

In the SQL editor, run migrations **in filename order**. If reports/blocks are already applied, you only need `20260919120000_direct_messages.sql`. It is additive: new table, extra SELECT policy on `user_blocks`, and `user_reports.target_type` also allows `'dm'`. No secrets, no client env beyond the existing anon key.
